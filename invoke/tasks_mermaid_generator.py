...
def get_final_tasks_from_listener(listener):
    ...
    export_nodes = [node['id']
                    for node in listener.nodes if node['type'] == 'E']
...
def generate_status_task(listener):
    ...
    for category, description, filename in status_files:
        status_entries.append(
            f"        ('{category}', '{description}', f'{filename}'),")
    status_list = '\n'.join(status_entries)
...
def generate_tasks_file(listener):
    ...
    def sort_tasks_by_dependencies(task_nodes, edges):
        ...
        while remaining_tasks:
            ...
            for task in remaining_tasks:
                dependencies = trace_task_dependencies(
            ...
            if not ready_tasks:
                ...
                print(
                    f"⚠️  Warning: Potential circular dependency, adding {remaining_tasks[0]['content']} anyway")
    ...
    for task_node in task_nodes:
        ...
        task_name = task_node['content']
        task_description = task_node.get(
            'description', task_name.replace('_', ' ').title())

        debug_task_mapping(task_id, listener.edges, listener.nodes)

        dependencies = trace_task_dependencies(
            task_id, listener.edges, listener.nodes)
...
def generate_meta_tasks(mermaid_file):
    ...
    try:
        ...
        print(
            f"✅ Parsed {len(listener.nodes)} nodes and {len(listener.edges)} edges")
...
Examples:
  ...
  python tasks_mermaid_generator.py --input TASKS.mmd --output generated_tasks.py
    ...
    if not input_path.suffix.lower() in ['.mmd', '.md']:
        print(
            f"⚠️  Warning: Input file doesn't have .mmd extension: {input_path}")
