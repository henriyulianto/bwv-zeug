...
def display_full_parsed_content(listener):
    ...
    # Nodes by type
    node_types = {'I': 'Inputs', 'T': 'Tasks',
                  'O': 'Outputs', 'R': 'Runnables', 'E': 'Exports'}
        ...
        if type_nodes:
            ...
            for node in type_nodes:
                ...
                if node_type in ['T', 'R']:
                    ...
                    inputs = [from_node for from_node,
                              to_node in listener.edges if to_node == node['id']]
                    if inputs:
                        print(f"      📥 Inputs: {', '.join(inputs)}")

                    # Show outputs (what flows out of this node)
                    outputs = [to_node for from_node,
                               to_node in listener.edges if from_node == node['id']]
                    if outputs:
            ...
            if producing_runnable:
                ...
                for from_node, to_node in listener.edges:
                    if to_node == producing_runnable and from_node.startswith('T'):
                        # Get node by ID
                        task_node = next(
                            (n for n in listener.nodes if n['id'] == from_node), None)
                        if task_node:
...
def get_status_file_info(mermaid_file):
    ...
    for output in file_info['outputs']:
        status_files.append(
            ('Output', output['description'], output['filename']))

    # Add exports
    for export in file_info['exports']:
        status_files.append(
            ('Export', export['description'], export['filename']))
