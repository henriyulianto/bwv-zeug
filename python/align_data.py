...
def main():
    ...
    try:
        ...
        expected_midi_columns = {"pitch", "midi",
                                 "channel", "on_tick", "off_tick"}
        if not expected_midi_columns.issubset(set(midi_df.columns)):
            raise ValueError(
                f"MIDI CSV missing required columns for tick format. Expected: {expected_midi_columns}, Found: {set(midi_df.columns)}")

        expected_svg_columns = {"snippet",
        ...
        print(
            f"   📐 Preserved {len(svg_df)} SVG noteheads in tolerance-based order")
        ...
        if len(midi_df) != len(svg_df):
            print(
                f"⚠️  Count mismatch: {len(midi_df)} MIDI events vs {len(svg_df)} SVG noteheads")
            print("   This may indicate that SVG data wasn't properly processed")
            print("   Continuing with available pairs for debugging...")
            print(
                f"   Will process {min(len(midi_df), len(svg_df))} matching pairs")
            print()
        ...
        for tick, midi_group in midi_groups:
            ...
            print(
                f"   🎵 Processing tick {tick}: {group_size} simultaneous events")

            # Try to match by pitch within the group
            midi_pitches = [lilypond_to_midi_pitch(
                row.pitch) for _, row in midi_group.iterrows()]
            svg_pitches = [lilypond_to_midi_pitch(
                row.snippet) for _, row in svg_group.iterrows()]
            ...
            if sorted(midi_pitches) == sorted(svg_pitches):
                ...
                midi_sorted = sorted(
                    midi_list, key=lambda x: lilypond_to_midi_pitch(x[1].pitch))
                svg_sorted = sorted(
                    svg_list, key=lambda x: lilypond_to_midi_pitch(x[1].snippet))
                ...
                print(
                    f"         MIDI pitches: {[midi_df.iloc[i].pitch for i in midi_group.index]} -> {midi_pitches}")
                print(
                    f"         SVG pitches:  {[svg_df.iloc[i].snippet for i in svg_group.index]} -> {svg_pitches}")
                print(
                    f"         Continuing with original order (voice crossing detected)")
        ...
        if group_mismatch_count > 0:
            print(
                f"   ⚠️  {group_mismatch_count} groups had pitch mismatches (voice crossing)")
        ...
        print(
            f"   ✅ Successfully processed {len(aligned_notes)} note alignments")
        if group_mismatch_count > 0:
            print(
                f"   ⚠️  Voice crossing detected in {group_mismatch_count} groups (continued anyway)")
        ...
        os.makedirs(os.path.dirname(output_json) if os.path.dirname(
            output_json) else '.', exist_ok=True)
        ...
        notes_with_ties = sum(
            1 for note in aligned_notes if len(note["hrefs"]) > 1)
        ...
        if group_mismatch_count > 0:
            print(
                f"⚠️  Voice crossing in {group_mismatch_count} groups (handled gracefully)")
