        ...
        if tie_role in ['start', 'both']:
            ...
            if tie_to:
                ...
                if start_data_ref:
                    ...
                    target_id = tie_to[1:] if tie_to.startswith(
                        '#') else tie_to
                        ...
                        if end_data_ref:
                            ...
                            if start_file == end_file:
                                ...
                                if is_valid_forward_tie(start_data_ref, end_data_ref):
                                    ...
                                    print(
                                        f"🔗 Found tie: {start_data_ref} → {end_data_ref}")
                                else:
                                    print(
                                        f"⚠️  Ignoring invalid backward tie: {start_data_ref} → {end_data_ref}")
                                    print(
                                        f"    (Ties must go forward in musical time)")
                            else:
                                print(
                                    f"⚠️  Ignoring invalid cross-file tie: {start_data_ref} → {end_data_ref}")
                                print(
                                    f"    (Ties cannot span different files: {start_file} vs {end_file})")
                        else:
                            print(
                                f"⚠️  Could not find data-ref for target element {target_id}")
                    else:
                        print(
                            f"⚠️  Could not find target element with id {target_id}")
                else:
                ...
                if first_row and (first_row[0].lower() == 'primary'):
                    ...
                    if first_row and len(first_row) >= 2:
                        ...
                        if (get_file_from_href(primary) == get_file_from_href(secondary) and
                            ...
                            print(
                                f"⚠️  Removing invalid existing tie: {primary} → {secondary}")
...
def save_ties_to_csv(ties, csv_file_path, existing_ties=None):
    ...
    print(
        f"💾 Saving {len(all_ties)} total ties ({new_ties_count} new) to: {csv_file_path}")

    # Ensure output directory exists
    os.makedirs(os.path.dirname(csv_file_path) if os.path.dirname(
        csv_file_path) else '.', exist_ok=True)
