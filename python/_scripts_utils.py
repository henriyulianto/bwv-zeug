...
def get_project_name():
    ...
    try:
        ...
        if Path(main_ly_file).exists():
            ...
            print(
                f"⚠️  Git repo name '{git_project_name}' doesn't match .ly file, trying directory name...")
...
def setup_project_context(script_purpose, input_pattern=None, output_pattern=None, extra_args=None):
    ...
    if not input_pattern and not output_pattern:
        if not project_name and (not args.input_file or not args.output_file):
            parser.error(
                "When PROJECT_NAME is not set, both input_file and output_file are required")
        elif project_name and not args.input_file:
            # Use project context to generate default positional args
            setattr(args, 'input_file', input_pattern.format(
                project=project_name) if input_pattern else None)
            setattr(args, 'output_file', output_pattern.format(
                project=project_name) if output_pattern else None)
