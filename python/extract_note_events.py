...
def extract_note_intervals(midi_path):
    ...
    # Track overlapping notes: {pitch: [(start_tick, channel), ...]}
    note_stack = {}
    ...
    print(
        f"   🎵 Processing {len(all_messages)} note messages from {len(midi_file.tracks)} tracks")
    ...
    for abs_tick, message in all_messages:
        ...
        # Handle note end events (note_off OR note_on with velocity=0)
            ...
            if note_stack.get(message.note):
                start_tick, channel = note_stack[message.note].pop(
                ...
                note_event = {
                    ...
                    # Start time in MIDI ticks (ensure integer)
                    "on_tick": int(start_tick),
                    # End time in MIDI ticks (ensure integer)
                    "off_tick": int(abs_tick),
                    "channel": channel
                }
    ...
    if note_events:
        ...
        print(
            f"   🔍 Tick verification: on_tick={sample_note['on_tick']} (type: {type(sample_note['on_tick'])})")
        print(
            f"   🔍 Tick verification: off_tick={sample_note['off_tick']} (type: {type(sample_note['off_tick'])})")
        ...
        if non_integers:
            print(
                f"   ⚠️  Warning: Found non-integer tick values in notes: {non_integers}")
        else:
    ...
    note_events_df = note_events_df[[
        "pitch", "midi", "channel", "on_tick", "off_tick"]]
    ...
    if len(note_events_df) > 0:
        ...
        for _, note in sample_notes.iterrows():
            print(
                f"      MIDI {note['midi']} -> '{note['pitch']}' ({note['on_tick']}-{note['off_tick']} ticks)")
...
def main():
    ...
    try:
        ...
        note_events_df.to_csv(output_file_path, index=False,
        ...
        unique_pitches = note_events_df["pitch"].nunique(
        ) if total_notes > 0 else 0
        unique_midi_pitches = note_events_df["midi"].nunique(
        ) if total_notes > 0 else 0
        channels_used = note_events_df["channel"].nunique(
        ) if total_notes > 0 else 0
        ...
        print(
            f"   🎼 LilyPond notation: {unique_pitches} unique representations")
