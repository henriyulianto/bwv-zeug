...
def load_noteheads_with_bars(csv_file: Path) -> Dict[str, Dict]:
    ...
    try:
        ...
        bar_count = sum(1 for n in noteheads.values()
                        if n['bar_moment'] is not None)
...
def merge_overlapping_bars(bars_info: Dict) -> Dict:
    ...
    for i, tick in enumerate(sorted_ticks):
        ...
        if len(bars_at_tick) == 1:
            ...
            merged_bars[first_bar_num] = {
                'start_tick': tick,
                # Use first bar's moment
                'start_moment': bars_at_tick[0][1]['start_moment'],
            ...
            for bar_num, bar_info in bars_at_tick:
                if 'noteheads' in bar_info:
                    merged_bars[first_bar_num]['noteheads'].extend(
                        bar_info['noteheads'])
    ...
    sorted_merged = sorted(
        merged_bars.keys(), key=lambda x: merged_bars[x]['start_tick'])
    ...
    print(
        f"   ✅ Merged {merge_count} bar groups, total bars after merge: {len(merged_bars)}")
    return merged_bars
...
def calculate_bar_durations(noteheads: Dict[str, Dict], sync_data: Dict, config_data: Dict = None) -> Dict:
    ...
    for i, bar_num in enumerate(sorted_bars):
        if i == len(sorted_bars) - 1:
            ...
            current_start_moment = parse_moment(
                bars_info[bar_num]['start_moment'])
            ...
            duration_beats = int(
                round((next_start_moment - current_start_moment) * 4))
    ...
    if len(sorted_bars) > 0:
        ...
        if config_data and 'musicalStructure' in config_data and 'lastMeasureDuration' in config_data['musicalStructure']:
            ...
            if '/' in str(last_measure_duration_str):
                ...
                last_measure_duration = int(
                    float(last_measure_duration_str) * 4)  # convert to beats
        ...
        if standard_duration_ticks is not None:
            if last_measure_duration is not None:
                ...
                current_start_moment = parse_moment(
                    bars_info[last_bar_num]['start_moment'])
                end_moment_quarter = int(
                    current_start_moment * 4) + last_bar_duration_beats
                bars_info[last_bar_num]['end_moment'] = f"{end_moment_quarter}/4"

                print(
                    f"   Last bar {last_bar_num}: Using config lastMeasureDuration ({last_bar_duration_ticks} ticks = {last_bar_duration_beats} beats)")
            else:
                ...
                current_start_moment = parse_moment(
                    bars_info[last_bar_num]['start_moment'])
                end_moment_quarter = int(
                    current_start_moment * 4) + standard_duration_beats
                bars_info[last_bar_num]['end_moment'] = f"{end_moment_quarter}/4"

                print(
                    f"   Last bar {last_bar_num}: Assuming standard duration ({standard_duration_ticks} ticks = {standard_duration_beats} beats)")
        else:
...
def assign_noteheads_to_beats(noteheads: Dict[str, Dict], sync_data: Dict, merged_bars: Dict, anacrusis_info: Dict = None) -> Tuple[Dict[str, Dict], List[Dict]]:
    ...
    for data_ref, notehead in updated_noteheads.items():
        ...
        if anacrusis_info and anacrusis_info['start_tick'] <= notehead_tick < anacrusis_info['start_tick'] + anacrusis_info['duration_ticks']:
            ...
            # 0/4 for first anacrusis beat
            beat_moment = f"{beat_in_anacrusis}/4"

            notehead['beat_moment'] = beat_moment

            # Check if exactly on beat
            expected_beat_tick = anacrusis_info['start_tick'] + \
                (beat_in_anacrusis * ticks_per_beat)
        ...
        expected_beat_tick = bar_info['start_tick'] + \
            (beat_in_bar * ticks_per_beat)
        is_on_beat = (notehead_tick == expected_beat_tick)
...
def verify_beat_counts_with_interpolation(all_beat_ticks: List[int], detected_beats: List[float], beat_assignments: List[Dict]) -> Dict:
    ...
    print(
        f"   Theoretical beats (from bar structure): {theoretical_beat_count}")
    ...
    if theoretical_beat_count == detected_beat_count:
        ...
        print(
            f"   ⚠️  Mismatch: {theoretical_beat_count} theoretical vs {detected_beat_count} detected beats")
    ...
    print(
        f"   📍 Anchor coverage: {anchor_coverage:.1f}% ({notehead_anchor_count}/{theoretical_beat_count})")
...
def map_beats_with_interpolation(verification: Dict, detected_beats: List[float]) -> Dict[int, int]:
    ...
    print(
        f"   Mapping {len(all_beat_ticks)} theoretical beats to {len(detected_beats)} detected beats")
    ...
    for i, (theoretical_tick, detected_beat_time) in enumerate(zip(all_beat_ticks, detected_beats)):
        ...
        if i < 10 or i >= len(all_beat_ticks) - 5:
            ...
            print(
                f"   Beat {i:3d}: tick {theoretical_tick:5d} → {new_tick:5d} ({detected_beat_time:.3f}s) [{status}]")
        elif i == 10:
            anchor_count = sum(
                1 for tick in all_beat_ticks[:i] if tick in anchor_ticks)
            interp_count = i - anchor_count
            print(
                f"   ... showing {anchor_count} anchors, {interp_count} interpolated ...")
    ...
    print(
        f"   📊 Final mapping: {total_anchors} anchored + {total_interpolated} interpolated = {len(beat_mapping)} total beats")
...
def interpolate_non_beat_ticks(sync_data: Dict, beat_tick_mapping: Dict[int, int]) -> Dict[int, int]:
    ...
    for old_tick in sorted(non_beat_ticks):
        ...
        if interpolated_count <= 10:
            print(
                f"   Tick {old_tick:5d} → {new_tick:5d} (between {o0_old}-{o1_old} → {o0_new}-{o1_new})")
...
def apply_tick_mappings_to_flow(sync_data: Dict, complete_tick_mapping: Dict[int, int], detected_beats: List[float] = None) -> Dict:
    ...
    for i, flow_item in enumerate(audio_sync_data['flow']):
        if len(flow_item) >= 4:
            ...
            # Update flow item
            audio_sync_data['flow'][i] = [
                new_start_tick, channel, new_end_tick, info]
            ...
            if new_start_tick != start_tick or (new_end_tick != end_tick and end_tick is not None):
                ...
                if transformed_items <= 5:
                    if end_tick is not None:
                        print(
                            f"   Item {i}: [{start_tick}, {channel}, {end_tick}] → [{new_start_tick}, {channel}, {new_end_tick}]")
                    else:
                        print(
                            f"   Item {i}: [{start_tick}, {channel}, {end_tick}] → [{new_start_tick}, {channel}, {new_end_tick}]")
                elif transformed_items == 6:
                    print(
                        f"   ... and {len(audio_sync_data['flow']) - 5} more transformations")
            else:
                unchanged_items += 1
    ...
    if 'meta' in audio_sync_data and complete_tick_mapping:
        ...
        if all_new_ticks:
            ...
            print(
                f"   🔄 Updated metadata: minTick={audio_sync_data['meta']['minTick']}, maxTick={audio_sync_data['meta']['maxTick']}")
    ...
    if 'meta' in audio_sync_data and detected_beats and len(detected_beats) > 0:
        ...
        print(
            f"   🎵 Updated musicStartSeconds to {first_beat_time:.3f}s (first detected beat)")
...
def save_audio_synced_yaml(audio_sync_data: Dict, output_yaml: Path):
    ...
    try:
        with open(output_yaml, 'w') as f:
            ...
            meta_data = {k: v for k, v in audio_sync_data.items()
                         if k != 'flow'}
            yaml_content = yaml.dump(
                meta_data, default_flow_style=False, sort_keys=False)
            ...
            for item in audio_sync_data.get('flow', []):
                ...
                for val in item:
                    if val is None:
                        ...
                        formatted_list = [f"'{str(v)}'" if isinstance(
                            v, str) else str(v) for v in val]
...
def main():
    parser = argparse.ArgumentParser(
        description='Generate audio-synchronized timing data for dynamic tempo playback')
    parser.add_argument(
        'noteheads_csv', help='Input noteheads CSV with bar timing')
    ...
    parser.add_argument(
        '-c', '--config', help='Configuration YAML file (optional)')
    parser.add_argument(
        '-o', '--output', help='Output audio-synced YAML file (default: sync_yaml with _audio suffix)')
    ...
    if args.output:
        ...
        output_yaml = sync_yaml.with_name(
            sync_yaml.stem + '_audio' + sync_yaml.suffix)
    ...
    _, beat_assignments = assign_noteheads_to_beats(
        noteheads, sync_data, merged_bars, anacrusis_info)

    # Step 4: Verify theoretical beats vs detected beats
    print("Step 4: Verifying theoretical vs detected beat counts...")
    verification = verify_beat_counts_with_interpolation(
        all_beat_ticks, detected_beats, beat_assignments)
    ...
    beat_tick_mapping = map_beats_with_interpolation(
        verification, detected_beats)

    # Step 6: Interpolate tick mappings for non-beat noteheads
    print("Step 6: Interpolating ticks for noteheads between beats...")
    complete_tick_mapping = interpolate_non_beat_ticks(
        sync_data, beat_tick_mapping)
    ...
    audio_sync_data = apply_tick_mappings_to_flow(
        sync_data, complete_tick_mapping, detected_beats)
