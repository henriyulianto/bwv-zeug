...
def collect_full_tie_group(primary_data_ref, ties_df):
    ...
    # Track visited notes to prevent infinite loops
    visited = set([primary_data_ref])
    # Notes whose ties we still need to check
    processing_queue = [primary_data_ref]
        ...
        for secondary_data_ref in direct_secondaries:
            if secondary_data_ref not in visited:
                ...
                processing_queue.append(
                    secondary_data_ref)  # Check its ties too
...
def main():
    ...
    try:
        ...
        if not expected_svg_columns.issubset(set(svg_df.columns)):
            raise ValueError(
                f"SVG CSV missing required columns. Expected: {expected_svg_columns}, Found: {set(svg_df.columns)}")

        expected_ties_columns = {"primary", "secondary"}
        if not expected_ties_columns.issubset(set(ties_df.columns)):
            raise ValueError(
                f"Ties CSV missing required columns. Expected: {expected_ties_columns}, Found: {set(ties_df.columns)}")
        ...
        if missing_data_ref_count > 0:
            print(
                f"   ⚠️  Found {missing_data_ref_count} rows with missing data_ref values - removing them")
            svg_df = svg_df.dropna(subset=['data_ref']).reset_index(drop=True)
            print(
                f"   ✅ Cleaned data: {len(svg_df)} valid noteheads remaining")
        ...
        secondary_data_refs = set(ties_df["secondary"]) if len(
            ties_df) > 0 else set()
        ...
        print(
            f"   Will embed {removed_count} secondary noteheads in tie groups")
        ...
        for index, row in primary_noteheads.iterrows():
            ...
            tied_secondaries = collect_full_tie_group(
                primary_data_ref, ties_df)
        ...
        tied_notes_count = sum(
            1 for data_refs in tied_data_refs_list if data_refs)
        total_tied_data_refs = sum(len(data_refs.split("|"))
                                   for data_refs in tied_data_refs_list if data_refs)
        ...
        if no_duplicates:
            ...
            for group_key in primary_noteheads['group_key'].unique():
                ...
                if len(group_rows) > 1:
                    ...
                    existing_tied = primary_noteheads.loc[primary_idx,
                                                          'tied_data_refs']
                    if pd.isna(existing_tied) or existing_tied == "":
                        combined_tied = "|".join(duplicate_data_refs)
                    else:
                        combined_tied = existing_tied + "|" + \
                            "|".join(duplicate_data_refs)

                    # Update the primary row
                    primary_noteheads.loc[primary_idx,
                                          'tied_data_refs'] = combined_tied
                    ...
                    print(
                        f"   🔄 Squashed {len(group_rows)} '{snippet}' notes at ({x:.1f}, {y:.1f})")

            # Remove duplicate rows while preserving order
            if rows_to_drop:
                primary_noteheads = primary_noteheads.drop(
                    rows_to_drop).reset_index(drop=True)
            ...
            if duplicates_squashed > 0:
                print(
                    f"   📊 Squashed {duplicates_squashed} duplicate noteheads")
                print(
                    f"   ✅ Final count: {len(primary_noteheads)} unique notes")
            else:
        ...
        print(
            f"   🎯 Preserved order for {len(primary_noteheads)} squashed noteheads")
        print(
            f"   ℹ️  Ordering was calculated with chord tolerance in extract_note_heads.py")
        ...
        output_df = primary_noteheads[["snippet",
                                       "data_ref", "x", "y", "tied_data_refs"]]
        ...
        if len(output_df) > 0:
            ...
            tied_examples = output_df[output_df["tied_data_refs"] != ""].head(
                3)
            if len(tied_examples) > 0:
                ...
                for _, example in tied_examples.iterrows():
                    ...
                    print(
                        f"      '{example['snippet']}' → {secondary_count} tied secondary(s)")
        else:
        ...
        if no_duplicates:
            print(
                "📝 Note: Duplicate noteheads at same position were merged into tied_data_refs")
        else:
            print(
                "📝 Note: Duplicate noteheads were preserved (duplicate squashing disabled)")
        ...
        if args.no_duplicates is None:
            ...
            print(f"\n💡 Configuration tip:")
            print(
                f"   To lock in this duplicate handling ({no_duplicates}) for project {project_name}:")
